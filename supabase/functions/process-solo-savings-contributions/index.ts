...
serve(async (req) => {
  ...
  try {
    ...
    // Get all active solo savings recurring contributions that are due
    const now = new Date()
    const { data: dueContributions, error: fetchError } = await supabase
      ...
      .select(`
        *,
        ...
        user:profiles!solo_savings_recurring_contributions_user_id_fkey(id, display_name, email),
    ...
    for (const contribution of dueContributions) {
      try {
        ...
        if (contribution.goal.current_amount >= contribution.goal.target_amount) {
          ...
          // Deactivate the recurring contribution
          await supabase
            .from('solo_savings_recurring_contributions')
        ...
        // Create Plaid transfer for the contribution
        try {
          console.log(`Creating Plaid transfer for $${contribution.amount} to goal: ${contribution.goal.name}`)
          
          // For now, we'll create a simple transfer request
          // In production, you'd want to get user details and device info from the request
          const transferRequest: TransferCreateRequest = {
            ...
            authorization_id: '', // This needs to be created first via plaid-transfer-authorization
            ...
            user: {
              ...
              phone_number: '', // Would need to get from user profile
          ...
          // 1. Call plaid-transfer-authorization first to get authorization_id
          // 2. Then call plaid-transfer with that authorization_id
          // 3. Handle the transfer status via webhooks
          
          console.log('Transfer request prepared:', transferRequest)
          
          // For now, we'll simulate a successful transfer
          // TODO: Implement actual Plaid transfer flow
        ...
        const { data: transaction, error: transactionError } = await supabase
          ...
          .insert({
            ...
            status: 'completed', // Assuming transfer succeeded
